...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _getBody(int index) {
    switch (index) {
      case 0: return _buildCharacterStats();
      case 1: return _buildHabitsList();
      case 2: return _buildTasksList();
      default: return _buildCharacterStats();
    }
  }
  ...
  // Существующие методы _buildCharacterStats() и _buildHabitsList() остаются без изменений
  // Добавим только новые методы для задач
  ...
  void _toggleTaskCompletion(Task task, bool completed) {
    setState(() {
      // Находим задачу в боксе и обновляем её
      final tasks = _getTasksFromBox(tasksBox);
  ...
  void _toggleHabitCompletion(Habit habit, bool completed) {
    setState(() {
      // Находим привычку в боксе и обновляем её
      final habits = _getHabitsFromBox(habitsBox);
